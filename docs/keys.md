...
| `←` / `h` | Collapse the selected directory. In the normal tree, on a **file** or an already-collapsed directory, walks up to and collapses the nearest visible parent, so repeated presses climb the tree. In changed-only (`c`) and status (`d`) views it keeps the existing collapse behavior, or **scroll the content pane left** when it is focused |
