...
| `p` | Pin the settled file preview as a frozen in-memory reference alongside the active preview; press again on that same file to remove it, or on another file to replace it |
| `{` / `}` | With a preview pinned, shrink / grow its horizontal share in 5-point steps (20–80%); the outer tree/content divider stays put. Inert with no pin |
